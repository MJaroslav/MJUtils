...
public class GuiImage implements ScrollingContentHeight {
    ...
    protected static GuiImage $build(@Nonnull CachedImage image, int width, int height, int extraYOffset, int zLevel, boolean packHeight) {
        Dimension modLogoDims = image.getDimensions();
        if (!image.isLoaded())
            return new GuiImage(width, height, extraYOffset, zLevel, 0, 0, 0, 0, image, true);
        if (width != 0 && height != 0) {
            ...
            return new GuiImage(width, height, extraYOffset, zLevel, contentWidth, contentHeight, contentX, contentY, image, packHeight);
        } else {
            return new GuiImage(width, height, extraYOffset, zLevel, modLogoDims.width, modLogoDims.height, 0, 0, image, true);
        }
    }

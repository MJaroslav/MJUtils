...
public class HooksBlockBreakingCreative {
    ...
    public static boolean onPlayerDestroyBlock(PlayerControllerMP instance, int x, int y, int z, int i) {
        return instance.isInCreativeMode() &&
                UtilsInteractions.isBlockBreakingDisabledInCreative(Minecraft.getMinecraft().thePlayer.getHeldItem());
    }
...
//            preCancelEvent = true;
        ...
        if (gameType.isCreative() && entityPlayer.getHeldItem() != null &&
                UtilsInteractions.isBlockBreakingDisabledInCreative(entityPlayer.getHeldItem()))
            preCancelEvent = true;
        // HOOK END

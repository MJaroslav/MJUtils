...
public class AnvilRecipe {
    ...
    /**
     * @param leftStack  - see {@link #leftStack}.
     * @param rightStack - see {@link #rightStack}.
     * @see AnvilRecipe
     */
     ...
     * @param leftStack  - see {@link #leftStack}.
     * @param rightStack - see {@link #rightStack}.
     * @param textField  - see {@link #textField}.
     * @see AnvilRecipe
     */
     ...
     * @param leftStack      - see {@link #leftStack}.
     * @param rightStack     - see {@link #rightStack}.
     * @param leftStackName  - see {@link #leftStackName}.
     * @param rightStackName - see {@link #rightStackName}.
     * @param textField      - see {@link #textField}.
     * @see AnvilRecipe
     */
     ...
     *
     * @param recipe - recipe for testing.
     * @return True if this object is the same as the obj argument; false
     * otherwise.
     */
    public boolean equals(AnvilRecipe recipe) {
        return recipe != null
                && itemStackNotNullAndEquals(this.leftStack, recipe.leftStack)
                ...
                && string(this.textField).equals(string(recipe.textField))
     ...
     * @param rightStack - see {@link #rightStack}.
     * @param textField  - see {@link #textField}.
     * @param cost       - see {@link #cost}. If it is minus one, it is not taken into
     *                   account.
     * @return True if this object is the same as the obj argument; false
     * otherwise.
     */
    public boolean equals(ItemStack leftStack, ItemStack rightStack, String textField, int cost) {
        return itemStackNotNullAndEquals(this.leftStack, leftStack)
                ...
                && (isNullOrEmpty(this.leftStackName) || string(nameFormat(leftStack)).equals(string(this.leftStackName)))
                && (isNullOrEmpty(this.rightStackName) || string(nameFormat(rightStack)).equals(string(this.rightStackName)))
                && (isNullOrEmpty(this.textField) || string(this.textField).equals(string(textField)))
                && (cost == -1 || this.cost == cost);
    }

    public AnvilRecipe copy() {
        return new AnvilRecipe(leftStack.copy(), rightStack.copy(), leftStackName, rightStackName,
                textField, cost);
    }
}
